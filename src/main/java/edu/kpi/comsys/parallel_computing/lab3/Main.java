...
import edu.kpi.comsys.parallel_computing.lab3.callable.CallableA;
import edu.kpi.comsys.parallel_computing.lab3.callable.CallableC;
import edu.kpi.comsys.parallel_computing.lab3.callable.CallableMA;
import edu.kpi.comsys.parallel_computing.lab3.callable.CallableMF;
import edu.kpi.comsys.parallel_computing.lab3.data.Matrix;
import edu.kpi.comsys.parallel_computing.lab3.data.Vector;
import edu.kpi.comsys.parallel_computing.lab3.util.FileUtil;
...
//        fileUtil.generateFile();
        ...
        for (int i = 0; i < p; i++) {
            PreThread thread = new PreThread(i, countDownLatch);
            thread.start();
        }
        ...
        for (int i = 0; i < p - 1; i++) {
            ...
            service.submit(new CallableMA(i, operations, barrier));
            service.submit(new CallableMF(i, operations, barrier));
        }
        futureA = service.submit(new CallableA(p - 1, operations, barrier));
        futureC = service.submit(new CallableC(p - 1, operations, barrier));
        futureMA = service.submit(new CallableMA(p - 1, operations, barrier));
        futureMF = service.submit(new CallableMF(p - 1, operations, barrier));
